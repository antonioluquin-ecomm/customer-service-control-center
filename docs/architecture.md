...
PENDING_QUEUE — operaciones offline tipadas (`create` / `delete`) pendientes de sincronizar
...
[Usuario completa formulario]
  → submitAuditoria()
    → postSheets(payload)
        ...
        └─ Error → cola `create` con `client_request_id` → savePendingQueue()

[Usuario retoma conexión]
  → retrySyncPending()
    → postSheets() para cada operación `create` / `delete` de PENDING_QUEUE
```
