...
function calificacionRuleText(){return `Excelente: ${CFG.u_excelente}% a 100% | Correcta: ${CFG.u_correcta}% a ${CFG.u_excelente-1}% | Observada: menos de ${CFG.u_correcta}%`;}
function renderCriterios(){ if(!CRITERIOS.length) CRITERIOS=JSON.parse(JSON.stringify(CRITERIOS_DEFAULT)); _criteriosSnapshot=JSON.parse(JSON.stringify(CRITERIOS)); const active=activeCriterios(),rule=document.getElementById('sc-calificacion-regla'); if(rule)rule.textContent=calificacionRuleText(); [['comunicacion','Comunicacion'],['gestion','Gestion']].forEach(([id,bloque])=>{const el=document.getElementById('criterios-'+id); if(el){el.innerHTML='';active.filter(c=>c.bloque===bloque).forEach(c=>el.appendChild(makeCriterioRow(c)));}}); updateCalidadTotal(); }
...
function renderResumen(){const cal=getCalidad(),estado=calcEstado(cal),h=escapeHtml;document.getElementById('resumen-content').innerHTML=`<div class="score-summary"><div><div class="ss-value" style="color:var(--accent)">${cal}%</div><div class="ss-label">Puntaje de calidad</div></div><div><div class="ss-value" style="color:var(--muted)">-</div><div class="ss-label">Productividad se carga aparte</div></div><div><div class="ss-value" style="color:var(--green)">${estado}</div><div class="ss-label">Estado de la muestra</div></div></div><div class="info-box" style="margin-bottom:16px"><strong>Regla de calificacion:</strong> ${calificacionRuleText()}</div><div class="resumen-grid"><div class="resumen-item"><div class="rl">Agente</div><div class="rv">${h(v('f-agente'))}</div></div><div class="resumen-item"><div class="rl">Ticket</div><div class="rv">${h(v('f-ticket'))}</div></div><div class="resumen-item"><div class="rl">Auditor</div><div class="rv">${h(v('f-auditor'))}</div></div><div class="resumen-item"><div class="rl">Semana</div><div class="rv">${h(v('f-semana'))}</div></div></div>`;}
